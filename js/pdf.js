async function generateInvoice() {
  ...
  // 1. Insert invoice WITHOUT invoice_number
  // serial_id is auto-generated by Supabase as int8 identity
  ...
  if (error || !data || !data[0] || !data[0].serial_id) {
    alert("Failed to save invoice");
    return;
  }

  // 2. Use returned serial_id for the invoice number
  const serialId = data[0].serial_id;
  const invoiceNumber = 'INV-' + String(serialId).padStart(4, '0');

  // 3. Optionally update the invoice_number column (store formatted invoice number)
  await supabaseClient
    .from('invoices')
    .update({ invoice_number: invoiceNumber })
    .eq('serial_id', serialId);

  // 4. PDF layout as per Indian GST Invoice Format
  const docDefinition = {
    content: [
      [
